...
export class MeetingRoom implements OnInit, OnDestroy {
  ...
  readonly userName = computed(() => {
    ...
    if (configName && configName !== 'Invitado') {
      return configName;
    }
    ...
    this.meetingService.getMeetingById(id).subscribe({
      ...
      error: (err) => {
        this.errorMessage.set(
          err.error?.message || 'La reunión no existe o no tienes acceso.'
        );
        this.isLoading.set(false);
      },
    });
  ...
  protected onToggleCamera(): void {
    this.isCameraOff.update((value) => !value);
  }
  ...
  protected toggleAgenda(): void {
    ...
    this.isFileViewerOpen.set(false);
    this.isAgendaOpen.update((value) => !value);
  }

  protected toggleFileViewer(): void {
    this.isChatOpen.set(false);
    this.isAgendaOpen.set(false);
    this.isFileViewerOpen.update((value) => !value);
  }
  ...
  ngOnDestroy(): void {
    this.chatSubs.forEach((subscription) => subscription.unsubscribe());
    this.signalingService.disconnect();
  }
}